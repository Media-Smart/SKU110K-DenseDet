...
def coco_eval(result_files,
    ...
    for res_type in result_types:
        ...
        cocoEval.params.iouThrs = np.linspace(.5, 0.95, 10, endpoint=True)
        #cocoEval.params.iouThrs = [0.5,0.75]
        
        cocoEval.params.maxDets = [100,300,400]
...
def fast_eval_recall(results,
                     coco,
                     max_dets,
                     iou_thrs=np.arange(0.5, 0.96, 0.05)):#,0.75]):#]np.arange(0.5, 0.96, 0.05)):
